...
export class UsersService {
    ...
    async findHosts(page = 1, limit = 20, specialty?: string) {
        ...
        let query = client
            .from('users')
            .select('id, name, email, specialty, description, address, is_active, created_at', { count: 'exact' })
    ...
    async findHostById(hostId: string) {
        ...
        const { data, error } = await client
            .from('users')
            .select('id, name, email, specialty, description, address, phone, is_active, created_at')
            .eq('id', hostId)
            .eq('role', UserRole.HOST)
            .eq('is_active', true)
            .single();
    ...
    async becomeHost(userId: string, hostData: { specialty: string; description?: string; address?: string }) {
        const client = this.supabase.getClient();
